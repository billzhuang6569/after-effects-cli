...
> **Windows**：以管理员身份运行命令提示符，执行 `reg add "HKCU\Software\Adobe\CSXS.11" /v PlayerDebugMode /t REG_SZ /d "1" /f`，重启 AE 即可。（注：CSXS.11 对应 AE 2024，不同版本数字可能不同）

### 第一步（补充）：开启 AE 脚本权限

在 AE 中打开：编辑（Windows）/ After Effects（macOS）→ 首选项 → 脚本和表达式，勾选 "允许脚本写入文件和访问网络"。

未开启此权限，插件面板无法写入桥接文件，所有操作将无法执行。
...
| `find_project_item` | 按精确名称或项目 item id 查找合成/素材/文件夹元数据 |
...
| `set_layer_switches` | 设置图层 solo、shy、3D、调整层等开关 |
| `batch_rename_layers` | 按规则批量重命名图层（前缀/后缀/查找替换） |
| `precompose_layers` | 将选中图层预合成 |
| `create_composition` | 新建合成 |
| `delete_composition` | 按精确名称或 item id 删除单个合成，用于临时合成清理 |
...
```
也可以使用 `id` / `itemId` 精确查找。重名时返回 `PROJECT_ITEM_AMBIGUOUS`，找不到时返回 `existed: false` 和 `PROJECT_ITEM_NOT_FOUND`。
...
```
也支持 `compName`、`id`、`itemId`。只会删除一个精确匹配的合成；如果同名合成超过一个，不会删除任何项目，并返回 `PROJECT_ITEM_AMBIGUOUS`。
